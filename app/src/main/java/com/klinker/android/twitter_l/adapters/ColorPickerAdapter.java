...
public class ColorPickerAdapter extends ArrayAdapter<ThemeColor> {
    ...
    public ColorPickerAdapter(Context context, List<ThemeColor> colors, View.OnClickListener itemClickedListener) {
        super(context, android.R.layout.simple_list_item_1, colors);
        this.colors = colors;
        this.currentColor = AppSettings.getInstance(context).themeColors.primaryColor;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        button.setInnerColor(color.primaryColor);
        button.setOuterColor(color.accentColor);
        frame.addView(button);

        if (color.primaryColor == currentColor) {
